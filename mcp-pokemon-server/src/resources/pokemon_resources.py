...
class PokemonResourceManager:
    ...
    def __init__(self, pokemon_client: PokemonAPIClient):
        ...
        logger.info("PokemonResourceManager initialized", 
    ...
    async def _handle_pokemon_info(self, params: List[str], query: Dict[str, List[str]]) -> str:
...
{species_data.flavor_text_entries[0]['flavor_text'].replace('\n', ' ') if species_data.flavor_text_entries and len(species_data.flavor_text_entries) > 0 else 'No description available.'}
...
"""
            ...
            # Add generation info
            generation = species_data.generation['name'] if species_data.generation else 'unknown'
            content += f"\n## Generation\n{generation.replace('-', ' ').title()}\n"
            
            return content
            
        except Exception as e:
            logger.error("Error handling pokemon info resource", 
                        name_or_id=name_or_id, 
                        error=str(e))
            raise Exception(f"Could not fetch Pokemon info for '{name_or_id}': {str(e)}")
    
    async def _handle_pokemon_stats(self, params: List[str], query: Dict[str, List[str]]) -> str:
...
"""
            ...
            logger.error("Error handling pokemon stats resource", 
                        name_or_id=name_or_id, 
                        error=str(e))
            raise Exception(f"Could not fetch Pokemon stats for '{name_or_id}': {str(e)}")
    
    async def _handle_pokemon_moveset(self, params: List[str], query: Dict[str, List[str]]) -> str:
...
"""
            ...
            logger.error("Error handling pokemon moveset resource", 
                        name_or_id=name_or_id, 
                        error=str(e))
            raise Exception(f"Could not fetch Pokemon moveset for '{name_or_id}': {str(e)}")
    
    async def _handle_type_info(self, params: List[str], query: Dict[str, List[str]]) -> str:
...
"""
            ...
            # Strong against
            strong_against = type_data.get('damage_relations', {}).get('double_damage_to', [])
            if strong_against:
            ...
            weak_against = type_data.get('damage_relations', {}).get('half_damage_to', [])
            if weak_against:
            ...
            weak_to = type_data.get('damage_relations', {}).get('double_damage_from', [])
            if weak_to:
            ...
            logger.error("Error handling type info resource", 
                        type_name=type_name, 
                        error=str(e))
            raise Exception(f"Could not fetch type info for '{type_name}': {str(e)}")
    
    async def _handle_generation_info(self, params: List[str], query: Dict[str, List[str]]) -> str:
...
"""
            ...
            logger.error("Error handling generation info resource", 
                        gen_number=gen_number, 
                        error=str(e))
            raise Exception(f"Could not fetch generation info for '{gen_number}': {str(e)}")
    
    async def _handle_pokemon_comparison(self, params: List[str], query: Dict[str, List[str]]) -> str:
...
"""
            ...
            winner = pokemon1_data.name.title() if total1 > total2 else pokemon2_data.name.title() if total2 > total1 else "Tie"
...
"""
            ...
            logger.error("Error handling pokemon comparison resource", 
                        pokemon1=pokemon1_name,
                        pokemon2=pokemon2_name,
                        error=str(e))
            raise Exception(f"Could not compare '{pokemon1_name}' and '{pokemon2_name}': {str(e)}")
