...
from ..models.pokemon_models import Pokemon, PokemonSpecies, PokemonSearchResult
...
class PokemonAPIClient:
    ...
    async def _make_request(self, endpoint: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 404:
                ...
                raise PokemonAPIError(f"API error {response.status_code}: {response.text}")
    ...
    async def get_pokemon(self, identifier: str) -> Pokemon:
        ...
        try:
            ...
            logger.info("Pokemon fetched successfully", name=pokemon.name, id=pokemon.id)
            return pokemon
    ...
    async def get_pokemon_species(self, identifier: str) -> PokemonSpecies:
        ...
        try:
            ...
            logger.info("Pokemon species fetched successfully", name=species.name, id=species.id)
            return species
        except Exception as e:
            logger.error("Failed to fetch Pokemon species", identifier=identifier, error=str(e))
            raise
    
    async def search_pokemon(self, limit: int = 20, offset: int = 0) -> PokemonSearchResult:
        ...
        try:
            ...
            logger.info("Pokemon search successful", count=result.count, returned=len(result.results))
            return result
        except Exception as e:
            logger.error("Failed to search Pokemon", limit=limit, offset=offset, error=str(e))
            raise
    ...
    async def get_multiple_pokemon(self, identifiers: List[str]) -> List[Pokemon]:
        ...
        logger.info("Multiple Pokemon fetch completed", 
...
_client: Optional[PokemonAPIClient] = None
